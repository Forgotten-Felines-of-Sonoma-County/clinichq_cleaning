...
def calculate_current_age(record_date_str, original_years, original_months):
    ...
    total_months = (years * 12 + months) + \
        (original_years * 12 + original_months)
...
def validate_microchip(microchip):
    """
    Validate microchip string format:
    - Must not be empty
    - Must be 15 characters or less
    - Must be alphanumeric only (no spaces, periods, hyphens, or other special characters)
    """
    if not microchip or not isinstance(microchip, str):
        return False, "Missing or invalid microchip"
    ...
    if not microchip:
        return False, "Missing microchip"

    # Check length
    if len(microchip) > 15:
        return False, f"Microchip too long ({len(microchip)} chars)"

    # Check for invalid characters (allow only alphanumeric)
    if not microchip.isalnum():
        return False, "Microchip contains invalid characters"

    return True, microchip
...
def process_clinic_data():
    ...
    for i, data in enumerate(all_data, 1):
        ...
        microchip = str(data.get('microchipNumber', ''))
        ...
        # Validate microchip
        is_valid, microchip_result = validate_microchip(microchip)
        if not is_valid:
            issues.append(microchip_result)
        else:
            microchip = microchip_result

        # Check for required address fields
        required_address_fields = [
            'ownerAddressLine1', 'ownerCity', 'ownerState', 'ownerZip']
        missing_fields = [
            field for field in required_address_fields if not data.get(field)]
        if missing_fields:
            issues.append(f"Missing address fields: {
                          ', '.join(missing_fields)}")
        ...
        # Default to 0 if None or empty
        original_years = data.get('ageYears', 0) or 0
        # Default to 0 if None or empty
        original_months = int(data.get('ageMonths', 0) or 0)
